// adding next and previous question button
let questionArray = [
    {
...
let firstTime = true;
let nextCount = 0;
let ansCount = 0, previousCount = 0;
...
const nextQuestion = () => {
    ...
    index++;
        ...
        console.log(((ansCount + nextCount) == index), (previousCount <= 0));
        
    if (((ansCount + nextCount) == index) && (previousCount < 0)) {
        firstTime = true;
    }
    loadQuestion();
};

// 02 load previous question
...
const getAns = (item) => {
    ...
    if (item.value == data[5]) {
        item.parentElement.classList.add('correct')
        item.nextElementSibling.insertAdjacentText("beforeend", "     Correect");
        correct++;
    } else {
        item.parentElement.classList.add('incorrect')
        item.nextElementSibling.insertAdjacentText("beforeend", "     Inorreect");
        incorrect++
    }
    ansCount++
    nextCount--;
