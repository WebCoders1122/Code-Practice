export class workout {
    constructor(container) {
        ...
        this.localArray = JSON.parse(localStorage.getItem(workout.Local_array_key) || "[]");
        this.showUpdate()
    ...
    showUpdate() {
        ...
        const addData = (element) => {
            ...
            row.querySelector('#workout-options').addEventListener('change', ({ target }) => {
                element.work = target.value;
                this.saveToLocal();
            })
            row.querySelector('.duration').addEventListener('change', ({ target }) => {
    ...
    saveToLocal() {
        localStorage.setItem(workout.Local_array_key, JSON.stringify(this.localArray))
    }
    addNewEntry(data) {
