...
const Hobbies_Skills_form = ({ editable_skill, editable_hobby }) => {
  ...
  const skills_handler = (e) => {
    ...
    if (editable_skill.index !== "") {
      dispatch.dispatch_sh({ type: "update_skill", value: new_skill });
    } else {
      dispatch.dispatch_sh({ type: "add_skill", new_skill: new_skill });
  ...
  const hobbies_handler = (e) => {
    ...
    if (editable_hobby.index !== "") {
      dispatch.dispatch_sh({ type: "update_hobby", value: new_hobby });
    } else {
      dispatch.dispatch_sh({ type: "add_hobby", new_hobby: new_hobby });
  ...
  return (
    <form>
      <div className='inputs_hs'>
        ...
        />
        <button
          className={`buttons ${themeColor}`}
          onClick={hobbies_handler}>
          Add Hobbies
        </button>
        <input
          type='text'
