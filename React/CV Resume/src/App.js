...
const App = () => {
  ...
  const [editable_education, set_editable_education] =
    useState(initial_edu_editable);
  const [editable_skill, set_editable_skill] = useState({
    index: "",
    value: "",
  });
  const [editable_hobby, set_editable_hobby] = useState({
    index: "",
    value: "",
  });
  ...
  const education_Reducer = (education, action) => {
    switch (action.type) {
      case "add": {
        return [
          ...education,
          { id: education.length + 1, ...action.education },
        ];
      }
      case "update": {
        let index = education.findIndex(
          (object) => action.education.id == object.id
        );
  ...
  const [education, dispatch_education] = useReducer(
    education_Reducer,
    Academic
  );
  ...
  const skills_hobbies = {
    //initial state of skills and hobbies
    skills: Skills,
    hobbies: Hobbies,
  };
  ...
  const [fontObj, setFontObj] = useState({
    color: "",
    fontSize: "",
    fontStyle: "",
  });
    ...
    <>
      <FontContext.Provider value={fontObj}>
        <ThemeContext.Provider value={themeColor}>
          <DataContext.Provider value={{ education, experiences, skill_hobby }}>
            <DispatchContext.Provider
              value={{ dispatch_education, dispatch_exp, dispatch_sh }}>
              <div className={`container2 ${themeColor}`}>
                <Toggle
                  isChecked={darkMode}
                  changeHandler={darkHandler}
                />
                <Heading
                  class_Name='main-heading'
                  content='Change Font Properties'
                />
                <form>
              ...
              <div className={`container2 ${themeColor}`}>
                <Heading
                  class_Name='main-heading'
                  content='Resume Data Form'
                />
                <Heading
                  class_Name='sub-heading'
                  content='Academic Records:'
                />
                <Academic_form editable_education={editable_education} />
                <Heading
                  class_Name='sub-heading'
                  content='Experiences Records:'
                />
                <Experience_Form editable_experience={editable_experience} />
                <Heading
                  class_Name='sub-heading'
                  content='Hobbies and Skills:'
                />
                <Hobbies_Skills_form
              ...
              <div className={`container ${themeColor}`}>
                ...
                <div className='space-between section'>
                  ...
                  <div>
                    <Information
                      heading='Address'
                      value='House # xxxx, Near ABC road, City xxxx, Punjab, Pakisan'
                    />
                    <Information
                      heading='Phone #'
                      value='0315-xxxxxxxxx'
                    />
                  </div>
                </div>
