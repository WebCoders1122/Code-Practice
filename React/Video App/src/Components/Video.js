...
const Video = ({ video_data, children, edit_video, dispatch }) => {
    ...
    return (
        <div className='container'>
            <button className={`close ${theme}`} onClick={() => dispatch({ type: 'DELETE', payload: video_data.id })}>x</button>
            <button className={`edit ${theme}`} onClick={() => edit_video(video_data.id)}>Edit</button>
            <div className="pic">
